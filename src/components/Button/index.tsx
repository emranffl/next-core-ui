...
export interface ButtonParams {
   ...
   */
  internalRef?: Ref<HTMLAnchorElement> | LegacyRef<HTMLButtonElement> | string | null
}
...
const Button = ({
   ...
   */
  const iconSize =
    icon?.size ?? (size ? (size == "sm" ? 14 : size == "lg" ? 20 : mergedIcon.size) : mergedIcon.size)

  const ConditionalIcon = ({ iconPosition }: { iconPosition: keyof typeof BUTTON_ICON_POSITIONS }) => {
    return mergedIcon.name && mergedIcon.position == iconPosition && !spinnerVisibility ? (
      <Icon
...
export default memo(
  forwardRef((props: ButtonProps, ref: Ref<HTMLAnchorElement> | LegacyRef<HTMLButtonElement>) => (
